...
data = pd.read_csv('C:/Users/MASTER/Downloads/pokemon.csv')

# Selecionar as variáveis '' e 'idade' do DataFrame
df = data[['cp', 'hp']].dropna()
...
groups = df.groupby('cp')['hp']

# Calcular os intervalos de confiança para cada grupo
confidence_intervals = groups.apply(lambda x: confidence_interval(x) if len(x) > 1 else (np.nan, np.nan))
